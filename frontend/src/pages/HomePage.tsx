...
import { AlertTriangle, Phone, MapPin, Users, Heart, Send, Loader2, Wifi, WifiOff, Camera, X, CheckCircle } from 'lucide-react';
  ...
  const sendSOS = async (): Promise<void> => {
    ...
    if (!isOnline) {
      ...
      setSuccess({
        ...
        message: 'Your SOS has been queued and will be sent when you\'re back online.'
      });
      setLoading(false);
      setShowForm(false);
      return;
    ...
    try {
      ...
      setSuccess({
        queued: true,
        localId: queued.localId,
        message: 'Network error. Your SOS has been queued and will retry automatically.'
      });
    } finally {
      setLoading(false);
    }
        ...
        {success ? (
          ...
          <div className="text-center space-y-6 max-w-md">
            ...
            {success.shortCode && (
              <div className="bg-gray-800 p-4 rounded-lg">
                <p className="text-sm text-gray-400 mb-1">Your tracking code:</p>
          ...
          <div className="w-full max-w-md space-y-4">
            ...
            {locationError ? (
              <div className="bg-red-900/50 border border-red-700 p-3 rounded-lg text-sm">
                ...
                <button onClick={getLocation} className="text-red-300 underline text-xs mt-1">
              ...
              <div className="bg-green-900/30 border border-green-700 p-3 rounded-lg text-sm flex items-center gap-2">
                ...
                <span className="text-green-300">Location captured: {location.lat.toFixed(4)}, {location.lng.toFixed(4)}</span>
              </div>
            ) : (
              <div className="bg-yellow-900/30 border border-yellow-700 p-3 rounded-lg text-sm flex items-center gap-2">
                <Loader2 className="w-4 h-4 animate-spin text-yellow-400" />
                <span className="text-yellow-300">Getting your location...</span>
              </div>
            )}
          ...
          <div className="text-center space-y-8">
            ...
            <p className="text-gray-400 max-w-xs mx-auto">
              Press the button below to send your location and request help immediately
            </p>
