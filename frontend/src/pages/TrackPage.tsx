...
import { 
  AlertTriangle, Clock, MapPin, User, CheckCircle, Truck, 
  Radio, ArrowLeft, RefreshCw, Share2, Copy
} from 'lucide-react';
  ...
  const getStatusInfo = (status: Status): StatusInfo => {
    const infoMap: Record<Status, StatusInfo> = {
      new: {
        ...
        description: 'Your distress signal has been received. A rescuer will respond shortly.',
        icon: Radio,
        step: 1
      },
      claimed: {
        ...
        description: 'A rescuer has claimed your case and is preparing to help.',
        icon: User,
        step: 2
      },
      en_route: {
      ...
      <div className="h-full flex items-center justify-center bg-gray-900 p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-400 mb-6">
            We couldn't find a report with code "{code?.toUpperCase()}". 
        ...
        <div className={`${statusInfo.bgColor} border ${statusInfo.borderColor} rounded-xl p-6 mb-6`}>
          ...
          {report.claimedByName && (
            <div className="flex items-center gap-2 text-gray-300 mt-4 pt-4 border-t border-gray-700">
              ...
              <span>Rescuer: <strong>{report.claimedByName}</strong></span>
            </div>
          )}
        ...
        <div className="bg-gray-800 rounded-xl p-6 mb-6">
          ...
          <div className="space-y-4">
            {progressSteps.map((item) => {
              const isComplete = statusInfo.step > item.step || 
                                (statusInfo.step === item.step && report.status === 'rescued');
              const isCurrent = statusInfo.step === item.step && report.status !== 'rescued';
              ...
              return (
                <div key={item.step} className="flex items-center gap-3">
                  ...
                  >
                    {isComplete ? <CheckCircle className="w-5 h-5" /> : item.step}
                  </div>
                  <span className={`${isComplete || isCurrent ? 'text-white' : 'text-gray-500'}`}>
                    {item.label}
                  </span>
                </div>
              );
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className={`px-2 py-0.5 rounded text-xs uppercase ${
                ...
                report.severity === 'high' ? 'bg-red-700 text-red-100' :
                report.severity === 'medium' ? 'bg-yellow-600 text-yellow-100' :
