...
export const SMS_CODES: Record<string, SMSCodeInfo> = {
  ...
  'C': { message: 'Child/Children in danger', severity: Severity.CRITICAL, isFragile: true },
  'M': { message: 'Medical emergency', severity: Severity.CRITICAL, isMedical: true },
  ...
  'E': { message: 'Elderly person needs help', severity: Severity.HIGH, isFragile: true },
  'P': { message: 'Pregnant woman needs help', severity: Severity.CRITICAL, isMedical: true, isFragile: true },
  'I': { message: 'Injured person', severity: Severity.HIGH, isMedical: true },
  'T': { message: 'Multiple people trapped', severity: Severity.CRITICAL, peopleCount: 3 },
  // Combined codes
  'MC': { message: 'Medical emergency with child', severity: Severity.CRITICAL, isMedical: true, isFragile: true },
  'MI': { message: 'Multiple injured', severity: Severity.CRITICAL, isMedical: true, peopleCount: 2 },
  'BT': { message: 'Building collapse, people trapped', severity: Severity.CRITICAL, peopleCount: 3 },
  'WE': { message: 'Flooding with elderly', severity: Severity.CRITICAL, isFragile: true },
};
...
export function detectSeverity(text: string): Severity {
  const critical = /critical|dying|dead|fire|collaps|child|baby|pregnant|heart|breath|bleed/i;
...
export function detectPeopleCount(text: string): number {
  const numMatch = text.match(/(\d+)\s*(?:people|persons|adults|children|family)/i);
...
export function parseSMS(body: string): ParsedSMS | null {
  ...
  const fullMatch = text.match(/^(?:HELP|SOS)\s+LAT[:\s]*([-\d.]+)\s+LON[:\s]*([-\d.]+)(?:\s+MSG[:\s]*(.*))?$/i);
  if (fullMatch) {
    parsed = {
  ...
  if (!parsed) {
    const stdMatch = text.match(/^(?:HELP|SOS)\s+([-\d.]+)[,\s]+([-\d.]+)\s*(.*)$/i);
    if (stdMatch) {
  ...
  if (!parsed) {
    const compMatch = text.match(/^H\s+([-\d.]+)\s+([-\d.]+)(?:\s+([A-Z]+))?$/i);
    if (compMatch) {
