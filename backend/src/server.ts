...
app.get("/api/reports/:identifier", (req: Request, res: Response) => {
  ...
  if (!report) {
    report = Array.from(reports.values()).find(
      (r) => r.shortCode === identifier.toUpperCase()
    );
  }
...
app.get("/api/stats", (_req: Request, res: Response) => {
  ...
  const stats: Stats = {
    ...
    byStatus: {
      ...
      claimed: allReports.filter((r) => r.status === ReportStatus.CLAIMED)
        .length,
      enRoute: allReports.filter((r) => r.status === ReportStatus.EN_ROUTE)
        .length,
      arrived: allReports.filter((r) => r.status === ReportStatus.ARRIVED)
        .length,
      rescued: allReports.filter((r) => r.status === ReportStatus.RESCUED)
        .length,
      closed: allReports.filter((r) => r.status === ReportStatus.CLOSED).length,
    },
    bySeverity: {
      critical: allReports.filter((r) => r.severity === Severity.CRITICAL)
        .length,
...
app.post("/api/sms/incoming", (req: Request, res: Response) => {
  ...
  try {
    ...
    if (parsed) {
      ...
      console.log(
        `[SMS SOS] Created report ${shortCode} at ${parsed.lat},${parsed.lng}`
      );
...
app.post("/api/sms/send", async (req: Request, res: Response) => {
  const { reportId, message } = req.body as {
    reportId: string;
    message: string;
  };
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("rescuer:join", (data: RescuerJoinData) => {
    ...
    socket.emit(
      "reports:sync",
      Array.from(reports.values()).map(sanitizeReport)
    );
  });

  socket.on("user:track", (shortCode: string) => {
    socket.join(`report:${shortCode}`);
    const report = Array.from(reports.values()).find(
